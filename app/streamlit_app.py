...
def _render_backend_sidebar() -> None:
    ...
    st.sidebar.markdown("### 🖥️ Backend")

    hf_config_preview = _resolve_hf_config(
        secrets=_get_secrets_mapping(),
        environ=os.environ,
    )
    mode_kind = "warm" if hf_config_preview.hf_token else "off"
    st.sidebar.markdown(
        _status_pill_html(_describe_backend_mode(hf_config_preview), mode_kind),
    ...
    if isinstance(wake_status, dict):
        ...
        kind = "warm" if wake_status.get("is_warm") else "cold"
        icon = "✅" if wake_status.get("is_warm") else "⚠️"
        st.sidebar.markdown(
            _status_pill_html(
                f"{icon} {wake_status.get('message')} · {elapsed_s:.0f}s ago", kind
            ),
            unsafe_allow_html=True,
        )
...
def _inject_styles() -> None:
    """Inject a small, self-contained CSS block for the hero, pills, and buttons."""
    st.markdown(
        """
        <style>
        .hero {
        ...
        .status-pill.warm { background: rgba(16, 185, 129, 0.15); color: #059669; }
        .status-pill.cold { background: rgba(245, 158, 11, 0.15); color: #b45309; }
        .status-pill.off { background: rgba(148, 163, 184, 0.25); color: #475569; }
...
def _status_pill_html(text: str, kind: str) -> str:
    """Build a small colored status-pill span. `kind` is one of warm/cold/off."""
...
def main() -> None:
    ...
    st.markdown(
        """
        <div class="hero">
          <h1>🧭 Analytics Copilot — Text-to-SQL</h1>
          <p>Ask a question in plain English, get back a SQL query — generated by a
          Mistral-7B model fine-tuned with QLoRA specifically for text-to-SQL,
          served remotely via Hugging Face Inference.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    stat_cols[0].metric("Base model", "Mistral-7B")
    stat_cols[1].metric("Fine-tuning", "QLoRA · 4-bit")
    stat_cols[2].metric("SQL parse success", "100%")
    stat_cols[3].metric("Schema adherence", "97.5%")
    ...
    if generate_clicked:
        ...
        if sql_text is not None:
            with st.container(border=True):
                ...
                diagnostics = st.session_state.get("diagnostics_last_request")
                if isinstance(diagnostics, dict):
                    st.caption(
                        f"Provider: **{diagnostics.get('provider', '—')}** · "
                        f"{diagnostics.get('duration_ms', 0):.0f} ms"
                    )
