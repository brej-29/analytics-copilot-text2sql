...
- **2026-01-10** – Implemented an evaluation pipeline for the internal val split of `b-mc2/sql-create-context` and a secondary external validation harness on Spider dev (Task 4), using sqlglot-based normalization/metrics and offline-friendly `--mock` modes.
...
- **2026-01-10** – Added an evaluation subsystem (Task 4): SQL normalization and metrics utilities under `src/text2sql/eval/`, an inference helper (`src/text2sql/infer.py`), internal evaluation script (`scripts/evaluate_internal.py`), and external Spider dev evaluation script (`scripts/evaluate_spider_external.py`), along with documentation in `docs/evaluation.md` and updated `README.md` / `context.md`.